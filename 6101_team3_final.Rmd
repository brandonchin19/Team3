...
```{r}
approval_logit <- glm(action_taken ~ derived_race + derived_ethnicity + derived_sex + loan_amount + income, data = loans_for_modelling, family = "binomial")
summary(approval_logit)
xkablevif(approval_logit)
```

```{r}
approval_logit_1 <- glm(action_taken ~ derived_race, data = loans_for_modelling, family = "binomial")
...
These numbers don't look correct, possibly due to the need to recode action_taken into 0 = denied and 1 = approved.
