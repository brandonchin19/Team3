...
# confustion martrix 
```{r}
table(hmda_down$action_taken, predict > 0.5)

```
# Evaluate overdispersion
```{r}
deviance(hmda_glm)/df.residual(hmda_glm) 
```
If the ratio considerably larger than 1, then it indicates that we have an overdispersion issue. Calculating this ratio using our data example, we find that the ratio is larger than 1. It means we have overdispersion problem on our model.
