...
```{r}
summary(hmda_ca_cleaned$applicant_ethnicity_2)
```
```{r}
summary(hmda_ca_cleaned$applicant_ethnicity_3)
```

Recommend dropping all demographic columns after "1" because those fields are barely used.

```{r}
summary(hmda_ca_cleaned$denial_reason_2)
```

Recommend dropping all denial reason columns after "1" because those fields are barely used.

Recommend removing all "aus" columns because they're irrelevant

Should we remove all co-applicant data?
