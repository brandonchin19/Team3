...
```{r}
ggplot(hmda_ca_final_1,aes(x=derived_sex))+
  geom_boxplot(color="black", fill="purple")+
  labs(title="Boxplot of Derived Sex", x="Derived Sex", y="Frequency")
```

Men appear more frequently in the data than women.

```{r}
ggplot(hmda_ca_final_1,aes(x=derived_race))+
  geom_boxplot(color="black", fill="purple")+
  labs(title="Boxplot of Derived Race", x="Derived Race", y="Frequency")
```

White applicants show up more than other races.
...
qplot(data = hmda_ca_final_1,
      ...
      y = derived_race,
      color = action_taken,
      main="Loan amount vs. Race by Action taken")
```

Loan amounts and loan originations are higher for white borrowers; loan amounts for other races are lower, and loan rejection rates are higher particularly for Black or African American Borrowers. 
...
# Removing outliers from interest_rate and property_value
...
```{r}
loans <- outlierKD2(hmda_ca_final_no_outliers_2,income)
```
