...
<<<<<<< HEAD
#Question KS: is it possible to add action taken here? Not sure this is useful because our main binary variable is not here
=======

#Question KS: is it possible to add action taken here?
>>>>>>> 9a3ed0ad35b0c2e81cfd5c93284c7dbf70965012
...
```{r}
<<<<<<< HEAD
contable2 = table(loans$derived_sex, loans$action)
=======
...
<<<<<<< HEAD
#Chi-Square Test for Loan Approval and Ethncity: result=>reject the null of independence
=======
>>>>>>> 9a3ed0ad35b0c2e81cfd5c93284c7dbf70965012
```{r}
...
```
<<<<<<< HEAD
#Chi-Square Test for Loan Approval and Age: result=>reject the null of independence (there is an issue with results because, I think, some groups are underrepresented); look into how to fix this
=======
...
<<<<<<< HEAD
#Create a new binary variable called "actionb"; 1=Denied; 0=Approved because we are interested in denials for our question; select only needed variables for logit regression
=======
...
#subset variables for logit  regression
loan_reg<-loans%>%select(ethnicity, race, derived_sex, loan_amount, interest_rate,
                         property_value, actionb, income)
...
#Start the logistic regression model building: "probability of getting denied because 1= denial, and 0=approval in our model"
```{r}
ggplot(hmda_ca_final_1,aes(y=property_value))+
  geom_boxplot(color="black", fill="blue")+
  labs(title="Boxplot of Property Values", y="Property Values in dollars", x="Frequency")
...
With outliers removed, the numerical values are more normally distributed and better suited for analysis.
library(ISLR)
m1<-glm(actionb~income+race+derived_sex, family = binomial, data=loans )
summary(m1)
```
